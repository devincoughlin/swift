...
class TestDecimal : TestDecimalSuper {
    ...
    func test_Misc() {
        ...
        expectEqual(Decimal(1.234), abs(Decimal(-1.234)))
        var a = Decimal(1234)
        var result = Decimal(1234)
        expectEqual(.noError, NSDecimalMultiplyByPowerOf10(&result, &a, 1, .plain))
        expectEqual(Decimal(12340), result)
        result = Decimal(1234)
        expectEqual(.noError, NSDecimalMultiplyByPowerOf10(&result, &a, 2, .plain))
        expectEqual(Decimal(123400), result)
        a = Decimal(123400)
        expectEqual(.overflow, NSDecimalMultiplyByPowerOf10(&result, &a, 128, .plain))
        expectTrue(result.isNaN)
        a = Decimal(1234)
        expectEqual(.noError, NSDecimalMultiplyByPowerOf10(&result, &a, -2, .plain))
        expectEqual(Decimal(12.34), result)
        a = Decimal(12.34)
        expectEqual(.underflow, NSDecimalMultiplyByPowerOf10(&result, &a, -128, .plain))
        expectTrue(a.isNaN)
        a = Decimal(1234)
        expectEqual(.noError, NSDecimalPower(&result, &a, 0, .plain))
        expectEqual(Decimal(1), result)
        a = Decimal(8)
        expectEqual(.noError, NSDecimalPower(&result, &a, 2, .plain))
        expectEqual(Decimal(64), result)
        a = Decimal(-2)
        expectEqual(.noError, NSDecimalPower(&result, &a, 3, .plain))
        expectEqual(Decimal(-8), result)
        for i in -2...10 {
            for j in 0...5 {
                var actual = Decimal(i)
                var result = Decimal(i)
    ...
    func test_Round() {
        ...
        for testCase in testCases {
            ...
            var roundedNum = Decimal(start)
            NSDecimalRound(&roundedNum, &num, scale, mode)
            expectEqual(Decimal(expected), roundedNum)
